  ...
  res.json({
    ...
    accessProfiles: Object.fromEntries(accessProfiles.map((profile) => [
      profile.userId,
      {
        ...
        departmentId: userById.get(profile.userId)?.departmentId ?? 'dept-1',
        scheduleEmployeeId: userById.get(profile.userId)?.scheduleEmployeeId ?? undefined,
        templateId: profile.templateId,
        overrides: parseJson<Record<string, boolean>>(profile.overridesJson, {}),
        active: (userById.get(profile.userId)?.isActive ?? true) && profile.isActive,
