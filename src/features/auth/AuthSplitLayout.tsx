...
export const AUTH_MAIN_COLUMN_CLASS =
  'relative flex min-h-screen min-w-0 items-center justify-center overflow-x-hidden overflow-y-auto p-3 sm:p-6 lg:min-h-0';
...
export default function AuthSplitLayout({ children }: AuthSplitLayoutProps) {
  return (
    <div
      ...
      className="min-h-screen w-full max-w-full overflow-x-clip bg-background lg:grid lg:h-screen lg:grid-cols-[minmax(0,1.1fr)_minmax(380px,0.9fr)] lg:overflow-hidden xl:grid-cols-[minmax(560px,1.1fr)_minmax(480px,0.9fr)]"
    >
      {children}
    </div>
  );
