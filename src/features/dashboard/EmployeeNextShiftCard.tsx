...
interface EmployeeNextShiftCardProps {
  ...
  return (
    <Card className="h-full">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-base font-semibold text-text-primary">{t('employee.nextShift.title')}</h2>
      </div>
      {!occurrence ? (
        <p className="mt-6 text-sm text-text-secondary">{t('employee.nextShift.empty')}</p>
      ) : (
        <div
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <span className="rounded-full border border-current/20 bg-surface/70 px-2.5 py-1 text-xs font-semibold text-inherit">
              {occurrence.source === 'ot' ? t('employee.source.ot') : t('employee.source.schedule')}
            </span>
          </div>
          <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="flex items-center gap-2 rounded-btn border border-current/10 bg-surface/70 p-3 text-sm text-inherit">
              <MapPin className="h-4 w-4" aria-hidden="true" />
              <span>{occurrence.facility} · {occurrence.unit}</span>
            </div>
            <div
              className="flex items-center gap-2 rounded-btn border border-current/10 bg-surface/70 p-3 text-sm font-medium text-inherit"
              dir="ltr"
            >
