  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-7">
      {days.map((day) => (
        <article
          key={day.date}
          data-testid="personal-month-day"
          className="min-h-[132px] rounded-card border border-border bg-surface p-3 shadow-card"
        >
          <time dateTime={day.date} className="block border-b border-border pb-2 text-sm font-semibold text-text-primary">
            <span className="me-1 text-primary">{new Date(`${day.date}T12:00:00`).getDate()}</span>
          ...
          <div className="mt-2 space-y-1.5">
            {day.occurrences.map((occurrence) => (
              <button
                ...
                className="block min-h-11 w-full rounded-btn border px-2 py-1 text-start text-xs font-semibold text-inherit transition-opacity hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-primary/30"
                style={occurrenceShiftStyle(occurrence)}
              >
                {t(`employeeView.categories.${occurrence.category}`)}
                <span className="mt-0.5 block truncate text-[10px] font-normal text-inherit opacity-80" dir="ltr">
