...
export interface HospitalLogoProps {
  ...
  size?: 'sm' | 'md' | 'lg' | 'xl';
  /**
   * نمط الألوان (default للملفات ذات الخلفية الفاتحة، white للسطح المظلم أو الأزرق)
   */
  variant?: 'default' | 'white' | 'colored';
  /**
   * إظهار النص العربي والانجليزي بجانب الرمز
   */
  showText?: boolean;
  /**
   * نص مخصص للقسم (افتراضياً: الأشعة المقطعية)
   */
  subtitle?: string;
  className?: string;
}
...
export default function HospitalLogo({
  ...
  variant = 'default',
  showText = true,
  subtitle = 'قسم الأشعة المقطعية - CT Scan',
  className,
}: HospitalLogoProps) {
  // الأبعاد حسب الحجم
  const sizeMap = {
  ...
  return (
    <div className={cn('flex items-center gap-3.5 select-none', className)}>
      {/* الرمز البصري للشعار (SVG Logo Emblem) */}
      <div
        className={cn(
      ...
      >
        ...
        {/* رسم الـ SVG المخصص (Shield + Medical Cross + CT Rings + National Palm Accents) */}
        <svg
          viewBox="0 0 100 100"
          fill="none"
        ...
        >
          ...
          {/* الدرع الخارجي (National Guard Shield Outline) */}
          <path
          ...
          {/* موجات وأطواق الأشعة المقطعية (CT Scanner Rings) */}
          <circle
            cx="50"
            cy="46"
            r="28"
          ...
          <circle
            ...
            cy="46"
            r="22"
            stroke={isWhite ? '#0EA5E9' : '#60A5FA'}
            strokeWidth="1.2"
            opacity="0.6"
          />

          {/* التقاطع الطبي في منتصف الدرع (Medical Cross) */}
          <path
            d="M44 32 H56 V40 H64 V52 H56 V60 H44 V52 H36 V40 H44 V32 Z"
            fill="url(#crossGrad)"
            rx="2"
            className="drop-shadow"
          />

          {/* نبض القلب / نبضة الإشعاع داخل الصليب (Pulse Wave) */}
          <path
            d="M40 46 L45 46 L48 41 L52 51 L55 46 L60 46"
            stroke={isWhite ? '#2563EB' : '#10B981'}
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          />

          {/* سعف النخلة الرمزي أسفل الدرع (Saudi Guard Palm Accent) */}
          <path
            d="M40 76 C45 73, 50 71, 50 71 C50 71, 55 73, 60 76 C55 78, 50 80, 50 80 C50 80, 45 78, 40 76 Z"
            fill="url(#goldGrad)"
            opacity="0.9"
          />
        ...
        {/* لمسة نقطة خضراء تفاعلية (Active Status Pulse) */}
      ...
      {/* النص والهوية الوظيفية (Typography & Title) */}
      {showText && (
